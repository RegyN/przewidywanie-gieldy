...
def zrob_jeden_trening(l_warstw=2, l_kom_ukr=20, bias='true', l_komorek_we=20,
    ...
    if trybwartosci:
        nazwa = (typ + "W" + str(l_warstw) + "K" + str(l_kom_ukr) + "LW" + str(l_komorek_we) + "LR" + str('%.2F' % learn_rate) + "M" + str(
            '%.2F' % momentum)
                 + "B" + str(batch_size) + "A" + akt_przejsc + "LE" + str(l_epok) + "W")
    else:
        nazwa = (typ + "W" + str(l_warstw) + "K" + str(l_kom_ukr) + "LW" + str(l_komorek_we) + "LR" + str('%.2F' % learn_rate) + "M" + str(
            '%.2F' % momentum)
                 + "B" + str(batch_size) + "A" + akt_przejsc + "LE" + str(l_epok) + "T")
...
def zrob_model_lstm(akt_przejsc, bias, l_kom_ukr, l_kom_lstm, l_warstw, l_wejsc_sieci, l_wyjsc_sieci):
    ...
    model.add(LSTM(input_shape=(720, l_wejsc_sieci,), units=l_kom_lstm, return_sequences=True, use_bias=bias, activation=akt_przejsc))
...
def zrob_dane(sciezka_csv, trybwartosci):
    ...
    for i, pakiet in enumerate(dane_tren):
        ...
        if trybwartosci:
            pakiet_out = pakiet[dlug_pak + 5 * 24 - 1][1]
        else:
            a = wsp_a_reg_lin(pakiet)
            if a > 1 / 20000:
                pakiet_out = [0,0,0,0,1]
            elif a > 1 / 200000:
                pakiet_out = [0, 0, 0, 1, 0]
            elif a > -1 / 200000:
                pakiet_out = [0, 0, 1, 0, 0]
            elif a > -1 / 20000:
                pakiet_out = [0, 1, 0, 0, 0]
            else:
                pakiet_out = [1, 0, 0, 0, 0]
...
def main():
    ...
    while True:
        print("")
        wybor = input("1- uczenie sieci 2- wczytanie wykresu 3 - uruchom skrypt 4 - rysuj wykresy skryptu 0- wyjscie z programu")
        if int(wybor) == 1:
            ...
            zrob_jeden_trening(l_warstw=4, l_kom_ukr=20, bias='true', l_komorek_we=32,
                               ...
                               batch_size=60, l_epok=2, val_split=0.2, trybwartosci=False, typ='lstm')
            zrob_jeden_trening(l_warstw=1, l_kom_ukr=20, bias='true', l_komorek_we=32,
                               akt_przejsc='tanh', learn_rate=0.15, momentum=0.15, decay=0.0,
                               batch_size=60, l_epok=2, val_split=0.2, trybwartosci=False, typ='rnn')
            if False:
                zrob_jeden_trening(l_warstw=4, l_kom_ukr=20, bias='true', l_komorek_we=32,
                                   akt_przejsc='tanh', learn_rate=0.15, momentum=0.15, decay=0.0,
