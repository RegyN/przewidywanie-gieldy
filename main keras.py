...
def zrob_jeden_trening(l_warstw=2, l_komorek_lstm=20, bias='true', l_komorek_we=80, l_komorek_wy=80,
    ...
    nazwa = "W"+str(l_warstw)+"K"+str(l_komorek_lstm)+"LR"+str(learning_rate)+"M"+str(momentum)+"B"+str(batch_size)\
            + ".csv"
    zapisz_historie(history, nazwa)


def main() :
    zrob_jeden_trening(l_warstw=1, l_epok=1)
